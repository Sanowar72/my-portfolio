...
export default function ParticleBackground() {
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      // Animated gradient mesh blobs (simplified on mobile)
      const t = timeRef.current * 0.005;
      const blobs = isMobile
        ? [
            { x: width * 0.3 + Math.sin(t * 0.7) * 80, y: height * 0.3 + Math.cos(t * 0.5) * 60, r: 200, color: [99, 102, 241] },
            { x: width * 0.7 + Math.cos(t * 0.6) * 70, y: height * 0.7 + Math.sin(t * 0.8) * 80, r: 180, color: [6, 182, 212] },
          ]
        : [
      ...
      if (mouseRef.current.x > 0) {
        ...
        ctx.fillRect(
          mouseRef.current.x - glowSize, mouseRef.current.y - glowSize,
          glowSize * 2, glowSize * 2
        );
      }

      // Spawn data packets (less frequently on mobile)
      const packetInterval = isMobile ? 60 : 30;
      if (packetTimer > packetInterval && connections.length > 0) {
        packetTimer = 0;
      ...
      for (let i = packets.length - 1; i >= 0; i--) {
        ...
        if (isMobile) {
          // Simple dot on mobile
          ctx.beginPath();
          ctx.arc(px, py, 2, 0, Math.PI * 2);
          ctx.fillStyle = `${p.color} 0.8)`;
          ctx.fill();
        } else {
          // Packet glow on desktop
      ...
      const starInterval = isMobile ? 400 : 200;
      if (starTimer > starInterval + Math.random() * 300) {
        starTimer = 0;
